...
const wasmPath = process.argv[2] ?? "target/wasm32-unknown-unknown/release/rsh_tissue_wasm.wasm";
...
assert.equal(Buffer.from(wasmBytes.subarray(0, 4)).toString("hex"), "0061736d");

const { instance } = await WebAssembly.instantiate(wasmBytes, {});
const api = instance.exports;
for (const name of ["memory", "rsh_tissue_abi_version", "rsh_tissue_run", "rsh_tissue_output_ptr", "rsh_tissue_output_len"]) {
...
const backendCode = { none: 0, webgpu: 1, cuda: 2, npu: 3 }[profile.configuration.sidecar_backend];
assert.ok(Number.isInteger(backendCode));
const c = profile.configuration;
const status = Number(api.rsh_tissue_run(
  c.cells, c.ticks, c.geometry_samples, c.ds, c.phase_coupling,
  c.binding_diffusion, backendCode, c.sidecar_residual, c.residual_gate, c.qf_floor,
));
...
assert.ok(pointer <= api.memory.buffer.byteLength && length <= api.memory.buffer.byteLength - pointer, "tissue output span exceeds WASM memory");
const payload = JSON.parse(new TextDecoder().decode(new Uint8Array(api.memory.buffer, pointer, length)));
...
for (const key of ["pass_all", "pass_constitution", "pass_bounds", "pass_centre", "pass_qf_floor", "audit_chain_valid"]) {
...
assert.deepEqual(wasmReport, rustReport, "native Rust and WASM reports differ");
...
function comparePortable(actual, expected, path = "report") {
  ...
  if (expected && typeof expected === "object") {
    for (const [key, value] of Object.entries(expected)) comparePortable(actual[key], value, `${path}.${key}`);
    return;
  }
  assert.equal(actual, expected, `${path} mismatch`);
}
comparePortable(rustReport, pythonReport);
comparePortable(wasmReport, pythonReport);

assert.equal(pythonReport.receipt, profile.expected.python_reference_report_receipt);
assert.equal(pythonReport.seed_geometry_receipt, profile.expected.python_seed_geometry_receipt);
assert.equal(pythonReport.ticks[0].receipt, profile.expected.python_reference_first_tick_receipt);
assert.equal(pythonReport.ticks.at(-1).receipt, profile.expected.python_reference_last_tick_receipt);
...
console.log(JSON.stringify({
  ...
  rust_wasm_receipt_identical: rustReport.receipt === wasmReport.receipt,
