...
def make_receipt(report: VerifyReport) -> str:
    return hashlib.sha256(RECEIPT_DOMAIN + canonical_report_bytes(report)).hexdigest()


def verify(rows: Sequence[Sample], config: ModelConfig, centre_tolerance: float = 1.0e-12) -> VerifyReport:
    if len(rows) != config.samples:
        raise ValueError("row count does not match the validated model configuration")
    centre = rows[len(rows) // 2]
    kappas = [row.kappa for row in rows]
    taus = [row.tau for row in rows]
    radii = [row.radius for row in rows]
    gaps = [rows[index + 1].p - rows[index].p for index in range(len(rows) - 1)]
    ...
    for row in rows:
        ...
        frame_norm_error = max(frame_norm_error, *(abs(norm(vector) - 1.0) for vector in frame))
    ...
    kappa_violations = sum(not (0.0 <= value <= KAPPA_MAX + 1.0e-12) for value in kappas)
    tau_violations = sum(not (0.0 < value < 1.0) for value in taus)
    ...
    pass_frame = frame_norm_error <= 1.0e-12 and frame_orthogonality_error <= 1.0e-12
    ...
    report = VerifyReport(
        ...
        endpoint_separation=norm(sub(rows[-1].position, rows[0].position)),
        pass_centre=pass_centre,
        pass_kappa=pass_kappa,
        pass_tau=pass_tau,
        pass_sampling=pass_sampling,
        pass_frame=pass_frame,
        pass_all=pass_centre and pass_kappa and pass_tau and pass_sampling and pass_frame,
    )
    return VerifyReport(**{**asdict(report), "receipt": make_receipt(report)})


def build_and_verify(config: ModelConfig = ModelConfig()) -> tuple[tuple[Sample, ...], VerifyReport]:
    config = config.validate()
    rows = build_path(config)
    return rows, verify(rows, config)


def verify_parallel(config: ModelConfig, workers: int = 2) -> tuple[VerifyReport, tuple[VerifyReport, ...], bool]:
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        reports = tuple(executor.map(lambda _: build_and_verify(config)[1], range(workers)))
    return baseline, reports, all(report.receipt == baseline.receipt for report in reports)


def benchmark(config: ModelConfig, loops: int = 20) -> dict[str, float | int | str | bool]:
    if loops < 1:
...
def write_trace_csv(rows: Sequence[Sample], path: str | Path) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            [
                "p", "s", "x", "y", "z", "kappa", "tau",
                "tx", "ty", "tz", "nx", "ny", "nz", "bx", "by", "bz", "radius",
            ]
        )
        for row in rows:
            writer.writerow(
                [
                    format(value, ".17e")
                    for value in (
                        row.p, row.s, row.x, row.y, row.z, row.kappa, row.tau,
                        row.tx, row.ty, row.tz, row.nx, row.ny, row.nz,
                        row.bx, row.by, row.bz, row.radius,
                    )
                ]
            )
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for key, value in asdict(report).items():
            if isinstance(value, float):
                ...
                encoded = ";".join(format(item, ".17e") for item in value)
            else:
