...
from .constants import KAPPA_MAX, MODEL_NAME, PSI, RECEIPT_DOMAIN, VERSION
...
from .geometry import ModelConfig, logical_sample_indices
...
def _add_model_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("-n", "--samples", type=int, default=513, help="odd sample count (default: 513)")
    ...
    parser.add_argument("--kappa-fraction", type=float, default=0.85, help="fraction of the curvature bound")
    parser.add_argument("--tau-floor", type=float, default=0.22, help="minimum torsion")
    parser.add_argument("--tau-amplitude", type=float, default=0.13, help="half-width of torsion modulation")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="rsh",
        description="Robitaille–Slade Helix: bounded Frenet–Serret geometry and deterministic evidence",
    )
    ...
    for command, help_text in (
        ...
        if command in {"verify", "trace", "visual"}:
            command_parser.add_argument("-o", "--output", default="", help="output file")
        if command == "verify":
            command_parser.add_argument("--json", default="", help="optional JSON report path")
        if command == "parity":
            command_parser.add_argument("--workers", type=int, default=4)
        if command == "benchmark":
            command_parser.add_argument("--loops", type=int, default=20)

    sample_parser = subparsers.add_parser("sample", help="emit exact bounded logical sample indices")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
                ...
                json.dumps(
                    {
                        ...
                        "kappa_max": KAPPA_MAX,
                        "tau_interval": "(0, 1)",
                        "construction": "prescribed curvature/torsion -> Frenet-Serret integration -> midpoint coordinate normalisation",
        ...
        if args.command == "sample":
            indices = logical_sample_indices(args.logical_count, args.rendered_count)
        ...
        rows, report = build_and_verify(config)
        ...
        if args.command == "verify":
            ...
            print(f"  centre_parameter     = {report.centre_parameter:.6f}")
            print(f"  centre_error         = {report.centre_error:.3e}")
            print(f"  max_kappa / bound    = {report.max_kappa:.6f} / {report.kappa_bound:.6f}")
            print(f"  tau range            = [{report.min_tau:.6f}, {report.max_tau:.6f}]")
            print(f"  frame_norm_error     = {report.max_frame_norm_error:.3e}")
            print(f"  frame_orthogonality  = {report.max_frame_orthogonality_error:.3e}")
        ...
        if args.command == "receipt":
            ...
            print(f"replay_identical={str(replay_identical).lower()}")
            return 0 if replay_identical else 1

        if args.command == "parity":
            baseline, reports, parity_ok = verify_parallel(config, args.workers)
        ...
        if args.command == "benchmark":
            print(json.dumps(benchmark(config, args.loops), indent=2, sort_keys=True))
            return 0
